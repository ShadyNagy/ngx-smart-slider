...
let left = [
  ...
  group([
    ...
    query(':leave', [style({ transform: 'translateX(0%)' }), animate('{{inTiming}}s ease-out', style({ transform: 'translateX(100%)' }))], {
      optional: true,
    }),
  ]),
];
...
@Component({
  ...
  animations: [
    trigger('animSlider', [
      transition(':increment', right, {params: {inWidth: '100%', inHeight: '100%'}}),
      transition(':decrement', left, {params: {inWidth: '100%', inHeight: '100%'}}),
    ]),
  ],
})
export class SmartSliderComponent implements OnInit {
  ...
  @HostBinding('@animSlider') get animSlider() {
    return {value: this.itemIndex, params: {inWidth: this.width, inHeight: this._height, inTiming: this._transitionDuration}};
  }
